...
public:
    ...
    /* Do not allow copies */
    FH_PrecLand(const FH_PrecLand &other) = delete;
    FH_PrecLand &operator=(const FH_PrecLand&) = delete;

    // return singleton
    static FH_PrecLand *get_singleton()
    { return _singleton; }
    ...
    // Parameter getters
    FH_PrecLand_StateMachine::RetryStrictness get_retry_strictness() const { return static_cast<FH_PrecLand_StateMachine::RetryStrictness>(_strict.get()); }
    uint8_t get_max_retry_allowed() const { return _retry_max; }
    float get_min_retry_time_sec() const { return _retry_timeout_sec; }
    FH_PrecLand_StateMachine::RetryAction get_retry_behaviour() const { return static_cast<FH_PrecLand_StateMachine::RetryAction>(_retry_behave.get()); }
...
private:
    ...
    FH_PrecLand_Backend         *_backend;  // pointers to backend precision landing driver

    // write out PREC message to log:
    void Write_Precland();
    uint32_t last_log_ms;  // last time we logged

    static FH_PrecLand *_singleton; //singleton
};

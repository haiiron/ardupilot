...
#include "FH_PrecLand_Backend.h"
#include "FH_PrecLand_IRLock.h"
#include "FH_PrecLand_SITL_Gazebo.h"
#include "FH_PrecLand_SITL.h"
...
const AP_Param::GroupInfo FH_PrecLand::var_info[] = {
        ...
        // @User: Advanced
        AP_GROUPINFO_FLAGS("ENABLED", 0, FH_PrecLand, _enabled, 0, AP_PARAM_FLAG_ENABLE),
        ...
        // @User: Advanced
        // @Units: cdeg
        AP_GROUPINFO("YAW_ALIGN",    2, FH_PrecLand, _yaw_align, 0),
        ...
        // @User: Advanced
        // @Units: cm
        AP_GROUPINFO("LAND_OFS_X",    3, FH_PrecLand, _land_ofs_cm_x, 0),
        ...
        // @User: Advanced
        // @Units: cm
        AP_GROUPINFO("LAND_OFS_Y",    4, FH_PrecLand, _land_ofs_cm_y, 0),
        ...
        // @User: Advanced
        AP_GROUPINFO("EST_TYPE",    5, FH_PrecLand, _estimator_type, 1),
        ...
        // @Range: 0.5 5
        // @User: Advanced
        AP_GROUPINFO("ACC_P_NSE", 6, FH_PrecLand, _accel_noise, 2.5f),
        ...
        // @Increment: 0.01
        // @User: Advanced
        AP_GROUPINFO("CAM_POS", 7, FH_PrecLand, _cam_offset, 0.0f),
        ...
        // @User: Advanced
        // @RebootRequired: True
        AP_GROUPINFO("LAG", 9, FH_PrecLand, _lag, 0.02f), // 20ms is the old default buffer size (8 frames @ 400hz/2.5ms)
        ...
        // @Units: m
        // @User: Advanced
        AP_GROUPINFO("XY_DIST_MAX", 10, FH_PrecLand, _xy_max_dist_desc, 2.5f),
        // @Param: STRICT
        ...
        AP_GROUPINFO("STRICT", 11, FH_PrecLand, _strict, 1),
        ...
        // @Range: 0 10
        // @Increment: 1
        AP_GROUPINFO("RET_MAX", 12, FH_PrecLand, _retry_max, 4),
        ...
        // @Range: 0 20
        // @Units: s
        AP_GROUPINFO("TIMEOUT", 13, FH_PrecLand, _retry_timeout_sec, 4),
        ...
        AP_GROUPINFO("RET_BEHAVE", 14, FH_PrecLand, _retry_behave, 0),
        ...
        // @Range: 0 5
        // @Units: m
        AP_GROUPINFO("ALT_MIN", 15, FH_PrecLand, _sensor_min_alt, 0.75),
        ...
        // @Range: 0 50
        // @Units: m
        AP_GROUPINFO("ALT_MAX", 16, FH_PrecLand, _sensor_max_alt, 8),
        ...
        // @User: Advanced
        AP_GROUPINFO("OPTIONS", 17, FH_PrecLand, _options, 0),

        AP_GROUPEND
};

// Default constructor.
FH_PrecLand::FH_PrecLand()
{
    if (_singleton != nullptr) { AP_HAL::panic("FH_PrecLand must be singleton"); }
    _singleton = this;
...
// 착륙 컨트롤러 초기화 수행
// update_rate_hz는 업데이트 방법을 hz 단위로 호출하는 속도
void FH_PrecLand::init(uint16_t update_rate_hz)
{
    // 이미 실행된 경우 즉시 종료
    if (_backend != nullptr) { return; }

    // 타겟 정보 : TARGET_NEVER_SEEN으로 시작
    ...
    // lag 및 minimum of main loop rate 및 update_rate_hz 인수로부터 관성 버퍼 크기 계산
    ...
    if (_inertial_history == nullptr) { return; }

    // 센서 유형 기반 백엔드 인스턴스화
    switch ((Type)(_type.get()))
    {
        case Type::NONE: // no type defined
        default:
            return;
            
        case Type::IRLOCK: // IR Lock
            _backend = new FH_PrecLand_IRLock(*this, _backend_state);
            break;
            
#if CONFIG_HAL_BOARD == HAL_BOARD_SITL
        case Type::SITL_GAZEBO:
            _backend = new FH_PrecLand_SITL_Gazebo(*this, _backend_state);
            break;

        case Type::SITL:
            _backend = new FH_PrecLand_SITL(*this, _backend_state);
            break;
#endif
    }

    // backend 초기화
    if (_backend != nullptr) { _backend->init(); }
}

// FH_PrecLand 업데이트, 센서로부터 업데이트를 받을 수 있는 기회를 줌.
void FH_PrecLand::update(float rangefinder_alt_cm, bool rangefinder_alt_valid)
{
    // 활성화되지 않은 경우 즉시 종료
    if (_backend == nullptr || _inertial_history == nullptr) { return; }

    // 현재 속도 및 자세 보정을 history buffer에 추가
...
// 타겟 상태 확인
void FH_PrecLand::check_target_status(float rangefinder_alt_m, bool rangefinder_alt_valid)
{
    if (target_acquired()) // 타겟이 시야에 있다면,
    {
        _current_target_state = TargetState::TARGET_FOUND;
        return; // 타겟을 알고 있기에 조기 return
    }

    // 타겟이 시야에 없을 경우, 목표물이 시야에 있었지만 더 이상 보이지 않는, 목표물을 놓친 상태.
    if (_current_target_state == TargetState::TARGET_FOUND || _current_target_state == TargetState::TARGET_RECENTLY_LOST)
    { _current_target_state = TargetState::TARGET_RECENTLY_LOST; }

    else { _current_target_state = TargetState::TARGET_NEVER_SEEN; } // 표적을 발견한 적이 없음.

    // 타겟이 보이지 않음.
    // PrecLand 센서가 범위 내에 있는지 확인,
    // 작동하려면 유효한 거리 측정기가 필요함.
    if (!check_if_sensor_in_range(rangefinder_alt_m, rangefinder_alt_valid))
    {
        // 타겟이 범위내에 있지 않음. (기체가 너무 높거나 너무 낮음). 기체는 어떠한 종류의 착륙 재시도를 시도하지 않음.
    ...
    {
        // 최근에 발견된 근처인지 확인. 그렇지 않으면 상태가 "TARGET_LOST"로 강등됩니다
        Vector2f curr_pos;
...
// 지면에서 기체의 높이를 기준으로 목표물이 시야에 있는지 확인.
// 최소/최대 매개변수가 0이 아닌 경우 작동하려면 유효한 거리 측정기가 필요.
bool FH_PrecLand::check_if_sensor_in_range(float rangefinder_alt_m, bool rangefinder_alt_valid) const
{
    if (is_zero(_sensor_max_alt) && is_zero(_sensor_min_alt)) {
        // 센서 제한이 지정되지 않았음. 센서가 항상 범위 내에 있다고 가정. (?)
        return true;
    }

    if (!rangefinder_alt_valid) // rangefinder 상태이상, 높은 고도에 있을 수 있음.
    { return false; }

    if (rangefinder_alt_m > _sensor_max_alt && !is_zero(_sensor_max_alt))
    { return false; } // 대상으로부터 너무 멀리 떨어져 있을 때 재시도 트리거를 방지할 수 있음.

    if (rangefinder_alt_m < _sensor_min_alt && !is_zero(_sensor_min_alt))
    { return false; } // 대상에 매우 근접했을 때 재시도 트리거를 방지할 수 있음.

    return true; // 대상이 범위내에 있어야 함.
}

bool FH_PrecLand::target_acquired()
{
    if ((AP_HAL::millis()-_last_update_ms) > LANDING_TARGET_TIMEOUT_MS)
    {
        if (_target_acquired)
        { // 타겟을 잃을 때마다 한 번만 전송.
            gcs().send_text(MAV_SEVERITY_CRITICAL, "FH PrecLand: Target Lost");
        }
        // 오랜시간 센서 업데이트가 없음.
        // 타겟을 잃었을 가능성이 높음.
...
bool FH_PrecLand::get_target_position_cm(Vector2f& ret)
{
    if (!target_acquired())
    { return false; }
...
void FH_PrecLand::get_target_position_measurement_cm(Vector3f& ret)
{
    ret = _target_pos_rel_meas_NED*100.0f;
    return;
}

bool FH_PrecLand::get_target_position_relative_cm(Vector2f& ret)
{
    if (!target_acquired()) {
        return false;
    }
...
bool FH_PrecLand::get_target_velocity_relative_cms(Vector2f& ret)
{
    if (!target_acquired()) {
        return false;
    }
...
void FH_PrecLand::get_target_velocity_cms(const Vector2f& vehicle_velocity_cms, Vector2f& target_vel_cms)
{
...
void FH_PrecLand::handle_msg(const mavlink_landing_target_t &packet, uint32_t timestamp_ms)
{
    // run backend update
    if (_backend != nullptr) { _backend->handle_msg(packet, timestamp_ms); }
}
...
void FH_PrecLand::run_estimator(float rangefinder_alt_m, bool rangefinder_alt_valid)
{
        ...
        case EstimatorType::RAW_SENSOR: {
            ...
            if (construct_pos_meas_using_rangefinder(rangefinder_alt_m, rangefinder_alt_valid)) {
                if (!_estimator_initialized) {
                    gcs().send_text(MAV_SEVERITY_INFO, "FH PrecLand: Target Found");
...
void FH_PrecLand::check_ekf_init_timeout()
{
    if (!target_acquired() && _estimator_initialized) {
        ...
        if (AP_HAL::millis()-_last_update_ms > EKF_INIT_SENSOR_MIN_UPDATE_MS) {
            ...
            _estimator_initialized = false;
            gcs().send_text(MAV_SEVERITY_CRITICAL, "FH PrecLand: Init Failed");
            ...
            _target_acquired = true;
            gcs().send_text(MAV_SEVERITY_INFO, "FH PrecLand: Init Complete");
        }
    }
}

bool FH_PrecLand::retrieve_los_meas(Vector3f& target_vec_unit_body)
{
...
bool FH_PrecLand::construct_pos_meas_using_rangefinder(float rangefinder_alt_m, bool rangefinder_alt_valid)
{
...
void FH_PrecLand::run_output_prediction()
{
...
// precision Landing 항목 작성
void FH_PrecLand::Write_Precland()
{
    // 활성화되어 있지 않으면 즉시 종료
    if (!enabled()) { return; }
...
// singleton instance
FH_PrecLand *FH_PrecLand::_singleton;

namespace FH {
    FH_PrecLand *fh_precland()
    { return FH_PrecLand::get_singleton(); }
} // FH
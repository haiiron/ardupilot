...
void FH_PrecLand_SITL_Gazebo::init()
{ irlock.init(get_bus()); }

// update - give chance to driver to get updates from sensor
void FH_PrecLand_SITL_Gazebo::update()
{
    // update health
...
bool FH_PrecLand_SITL_Gazebo::get_los_body(Vector3f& ret) {
    if (have_los_meas())
    {
        ret = _los_meas_body;
        return true;
...
uint32_t FH_PrecLand_SITL_Gazebo::los_meas_time_ms()
{ return _los_meas_time_ms; }

// return true if there is a valid los measurement available
bool FH_PrecLand_SITL_Gazebo::have_los_meas()
